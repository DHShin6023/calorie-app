...
[중요 규칙]
1. 한식·일식·중식·양식·동남아식 등 어떤 나라 음식이든 정확히 식별하세요.
2. 메인 요리뿐 아니라 사이드 디시, 반찬, 음료, 국, 소스 등 사진에 보이는 모든 음식을 개별 식별하세요.
3. 외관의 색상·재료·조리법을 근거로 음식을 정확히 구분하세요:
   - 한식 예시: 김치볶음밥(빨간 볶음밥) vs 비빔밥(나물+달걀) vs 간장볶음밥(갈색)
   - 일식 예시: 라멘 vs 우동 vs 소바 / 스시 vs 사시미
   - 중식 예시: 짜장면(검은 소스) vs 짬뽕(빨간 국물)
